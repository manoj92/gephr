...
def handler(event, context):
    ...
    try:
        if path == '/' or path == '/health':
            body = {"message": "Humanoid Training Platform API", "version": "1.0.0", "status": "running"}
        elif path == '/api/v1/health':
            body = {"status": "healthy", "api_version": "v1", "timestamp": datetime.now().isoformat()}
        elif path == '/api/v1/robots':
            body = {"robots": robots, "total": len(robots)}
        elif path == '/api/v1/training/sessions':
            body = {"sessions": sessions, "total": len(sessions)}
        elif path == '/api/v1/marketplace':
            body = {"items": marketplace, "total": len(marketplace)}
        elif path == '/api/v1/tracking/status':
            body = {"tracking_active": True, "fps": 30, "latency_ms": 25, "hands_detected": 2}
        elif path == '/api/v1/stats':
            body = {"total_robots": 4, "active_sessions": 1, "completed_sessions": 2, "marketplace_items": 4, "total_downloads": 245, "platform_uptime": "99.9%"}
        else:
